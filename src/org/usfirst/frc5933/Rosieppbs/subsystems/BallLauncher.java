...
public class BallLauncher extends Subsystem {
    ...
    private static final double kMotorToInchScaler = 0.12; // FIXME: See note below.
    
    public static final double kSlowFlyWheeelSpeed = -0.40;
    public static final double kMediumFlyWheeelSpeed = -0.50;
    public static final double kFastFlyWheeelSpeed = -0.63;
    ...
    public void set(double speed, double idealDistance) {
        // FIXME: Replace (distanceDiff / kMotorToInchScaler) with value that is calculated via
        /// a slope intercept formula since we are assuming that the speed increases linearly.
        
        if (Robot.useLidarToAdjustFlyWheel) {
            double distanceDiff = idealDistance - spock.getDistance();
            if (distanceDiff == 0) {
                flyWheel.set(speed + getFlyWheelAdjustment());
            } else if (distanceDiff < 0) {
                flyWheel.set(speed - (distanceDiff / kMotorToInchScaler) + getFlyWheelAdjustment());
            } else {
                flyWheel.set(speed + (distanceDiff / kMotorToInchScaler) + getFlyWheelAdjustment());
            }
        } else {
